...
async function N(e, { content: t, path: i, url: r }) {
...
}, p = Object.entries(d).filter(([e, { deployed: t }]) => t == null || t).map(([e]) => e);
function b() {
...
function C(e) {
  ...
  if (!e) return t;
  const i = h(e);
  return t.startsWith("http://") || t.startsWith("https://") ? new URL(i, t).toString() : `${t === "/" ? "" : y(t)}/${i}`;
}
function A(e) {
  return C(`libraries/${h(e)}`);
}
const v = (e) => {
...
async function O(e) {
  const t = BrowserFS.BFSRequire("buffer").Buffer, i = async (n) => (await fetch(n)).arrayBuffer(), r = await Promise.all(
    e.map(async (n) => [n, await i(A(`${n}.zip`))])
  ), o = {};
  for (const [n, s] of r)
    o[n] = {
      fs: "ZipFS",
...
async function _({ prefix: e, allowPersistence: t }) {
  const r = await O(p), o = {};
  for (const s in r)
    o[`${e}${s}`] = r[s];
  await E(typeof window == "object" && window || self, {
    fs: "OverlayFS",
    options: {
      readable: {
...
//# sourceMappingURL=filesystem-D0H6E9q7.js.map
