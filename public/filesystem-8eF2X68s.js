...
async function v(e, { content: t, path: i, url: r }) {
...
function y() {
  if (typeof document > "u" && typeof self < "u" && self.location) {
    const e = new URL(self.location.href);
    let t = e.pathname.replace(/\/[^/]*$/, "/");
    return t = t.replace(/\/assets\/$/, "/"), `${e.origin}${t}`;
  }
}
function C() {
  return f() ?? g() ?? y() ?? S() ?? "/";
...
function w(e) {
  ...
  if (!e) return t;
  const i = h(e);
  return t.startsWith("http://") || t.startsWith("https://") ? new URL(i, t).toString() : `${t === "/" ? "" : A(t)}/${i}`;
}
function O(e) {
  return w(`libraries/${h(e)}`);
}
const x = (e) => {
...
function E(e, t) {
  return e === "." ? t : e.endsWith("/") ? E(e.substring(0, e.length - 1), t) : t === "." ? e : `${e}/${t}`;
}
async function D(e) {
...
async function F(e, t, i = "/libraries", r = "/tmp") {
  const o = async (n, s) => {
    try {
      await t.symlink(n, s);
    } catch (a) {
...
async function M({ prefix: e, allowPersistence: t }) {
  const r = await D(p), o = {};
  for (const s in r)
    o[`${e}${s}`] = r[s];
  await B(typeof window == "object" && window || self, {
    fs: "OverlayFS",
    options: {
      readable: {
...
//# sourceMappingURL=filesystem-8eF2X68s.js.map
